...
`Agent` / `ModelConfig` / `Conversation` / `Message` / `KnowledgeBase` / `Document` / `Chunk` / `Artifact` / `Workspace` / `AgentRun`

> `ModelConfig` 是一次 LLM 调用所需凭证的独立实体（`adapterName` / `provider` / `modelId` / `baseURL` / `apiKey`）。Agent 通过 `modelConfigId` **纯引用** 一条配置，不再内嵌模型字段。`adapterName` 跟随 ModelConfig（与 endpoint/key 强绑定）。恰有一条 `isDefault: true`，Agent 未指定时用它。
...
对话 LLM 凭证（`resolveCredentials`）：
1. **Agent 引用的 `ModelConfig`** — Agent.modelConfigId 指向的那条
2. **默认 `ModelConfig`** — Agent 未指定时用 `isDefault: true` 那条
3. **`process.env`** — ModelConfig 字段为空时 `.env.local` 兜底

> Embedding / Rerank 凭证独立：`app_settings` > `process.env`，不走 ModelConfig（模型在建 KB 时定死）。

约束：
- **绝不**在代码中硬编码 key
- 缺失 key 时，由 adapter 抛错（不要在启动时拒绝服务）
- **key 明文绝不发往前端**：ModelConfig 存 SQLite（明文，`data/` 已 gitignore），但所有面向前端的 API（GET/POST/PUT）必须经 `toModelConfigView` 脱敏，只回 `hasApiKey` 布尔标记。PUT 时 apiKey 留空视作"不修改"，避免误清。
